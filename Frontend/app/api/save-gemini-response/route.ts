...
export async function POST(request: NextRequest) {
  try {
    const { filename, content, championName, matchId, timestamp } = await request.json();

    // Create gemini-responses directory if it doesn't exist
    const responsesDir = join(process.cwd(), 'gemini-responses');
    await mkdir(responsesDir, { recursive: true });

    // Determine the analysis type and create appropriate content
    let fileContent;
    if (matchId) {
      // Match analysis
      fileContent = `# Gemini AI Match Analysis for ${matchId}
Generated: ${timestamp}
...
---
*This analysis was generated by Gemini AI for League of Legends match strategic improvement.*
`;
    } else {
      // Champion analysis
      fileContent = `# Gemini AI Analysis for ${championName}
Generated: ${timestamp}
